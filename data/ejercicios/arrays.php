<?php

    // los elementos son heterogéneos
    $miarray = array(1, "hola", 3.0, false);

    echo "<br> Elemento 2 del array: " . $miarray[2];
    echo "<br>";

    // mostrar
    // foreach, var_dump, print_r

    foreach($miarray as $elmto){
        echo "<br> el elmto es: " . $elmto;
    }

    echo "<br><br>";

    print_r($miarray);

    echo "<br><br>";

    var_dump($miarray);

    echo "<br><br>";

    echo "longitud del array: " . count($miarray);

    $array2 = [
        3,
        2,
        5,
        9
    ];

    foreach($array2 as &$elto){
        $elto = $elto * 2;
        echo "<br><br>";
        echo "valor de elto: " . $elto;
    }
    echo "<br><br>";
    print_r($array2);

    $array2 [] = 15;
    echo "<br><br>";
    print_r($array2);

    // eliminar un elemento
    unset($array2[2]);
    echo "<br><br>";
    print_r($array2);
    echo "<br> y la posicion 2? : " . $array2[2];

    echo "<br><br>";
    $array2 [2] = 13;
    print_r($array2);

    echo "<br> y la posicion 2? : " . $array2[2];

    echo "<br><br>";
    echo "MOSTRAR CLAVE Y VALOR DEL ARRAY: ";
    foreach($array2 as $key => $valor){
        echo "<br> Clave: " . $key . " y valor: " . $valor;
    }

    echo "<br><br>";
    echo "<br><br>";

    $persona = array(
        "edad" => 23,
        "peso" => 75,
        "casado" => false,
        "dni" => "111222333",
        8 => "nss"
    );

    echo "<br><br>";
    
    foreach($persona as $key => $value){
        echo "<br> Clave: " . $key . " y valor: " . $value;
    }
    echo "<br><br>";
    print_r($persona);

    $persona[] = 7;
    echo "<br>";
    print_r($persona);

    $persona[4] = "natacion";
    echo "<br>";
    print_r($persona);


    echo "<br><br>";
    echo "El dni es: " . $persona["dni"];

    // arrays multidimiensionales : matrices, arrays de arrays
    $arrm = [
        0 => [1,3],
        1 => [5,7,9],
    ];

    echo "<br> Debe salir 3 : " . $arrm[0][1];